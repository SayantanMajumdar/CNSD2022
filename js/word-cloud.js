(() => {
  ...
  SciTerms.sort(function(x2, y2) {
    return y2.freq - x2.freq;
  });
  var populatedTerms = [];
  ...
  function embedWord(term, x2, y2) {
    wordCloud.appendChild(term);
    term.style.left = x2 - term.offsetWidth / 2 + "px";
    term.style.top = y2 - term.offsetHeight / 2 + "px";
  ...
  var origin = {
    x: wordCloud.offsetWidth / 2,
    y: wordCloud.offsetHeight / 2
  };
  function intersect(word, x2, y2) {
    wordCloud.appendChild(word);
    word.style.left = x2 - word.offsetWidth / 2 + "px";
    word.style.top = y2 - word.offsetHeight / 2 + "px";
    var currentWord = word.getBoundingClientRect();
    wordCloud.removeChild(word);
    for (var i = 0; i < populatedTerms.length; i += 1) {
      var comparisonWord = populatedTerms[i];
      if (!(currentWord.right + 0 < comparisonWord.left - 0 || currentWord.left - 0 > comparisonWord.right + 0 || currentWord.bottom + 3 < comparisonWord.top - 3 || currentWord.top - 3 > comparisonWord.bottom + 3)) {
        return true;
      }
    }
    return false;
